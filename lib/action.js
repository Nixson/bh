...
var Action = function(globalData){
	return {
		...
		saveInfo: function(cid,uid,info,msg){
			_this.globalData.mysql.getConnection(function(_, connection) {
				connection.query('SELECT 1 from bh_customer where uid = ?', [uid],function(_, rows) {
					var post  = {sid: info.sid, info: JSON.stringify(info.info), msg: msg};
					if( typeof rows == 'undefined'){
						post.ctime = parseInt((new Date).getTime()/1000);
						post.uid = uid;
						connection.query("INSERT INTO bh_customer SET ?",post,function(_,_){
							connection.release();
						});
					}else {
		...
		saveInfoAndMsg: function(cid,uid,info){
			var _this = this;
				_this.globalData.redis.llen('bh:c:msg:'+uid,function(_,resp){
					if(resp > 0){
						_this.globalData.redis.lrange('bh:c:msg:'+juid,0,resp,function(err,respRange){
							var result = {};
							for(var nm in respRange){
								result[parseInt(nm)] = respRange[nm];
							}
							saveInfo(cid,uid,info,JSON.stringify(result));
						});
					}
					else {
						saveInfo(cid,uid,info,"");
					}
				});
		},
