					...
					if(resp!=''){
						...
						_this.globalData.redis.keys('bh:m:i:'+cid+':*',function(_, keys){
							if(typeof keys!='undefined' && keys.length == 0){
								var spi = spawn('/usr/bin/php', ['/opt/bh/html/cron/reconfigure.php',cid]);
								spi.stdout.on('data', function (data) {
								  console.log('stdout: ' + data);
								});

								spi.stderr.on('data', function (data) {
								  console.log('stderr: ' + data);
								});

								spi.on('exit', function (code) {
		...
		managerSendOut: function(mkey,cid,uid){
			...
			_this.cmdUID(function(cmuid){
				...
				_this.globalData.redis.get(mkey,function(_,resp){
					var info = JSON.parse(resp);
					process.send({ manager: {type: "cmd", cid: cmuid, uid: info.uid}});
				});
			});
		},
