			...
			if(typeof msg.client != 'undefined'){
				switch(msg.client.type){
					...
					case "exit":
						...
						break;
					case "clear":
						if(Object.keys(gData.clients[msg.client.uid]).length==0){
			...
			if(typeof msg.manager != 'undefined'){
				switch(msg.manager.type){
					case "in":
						...
						if(typeof gData.managers[msg.manager.uid][msg.client.cluster] == 'undefined') gData.managers[msg.manager.uid][msg.manager.cluster] = 0;
						gData.managers[msg.client.uid][msg.client.cluster]++;
						break;
					case "exit":
						...
						break;
					case "clear":
						if(Object.keys(gData.managers[msg.manager.uid]).length==0){
			...
			switch(info.type){
				case "client": 
					if(typeof gData.clients[info.uid]=='undefined' || Object.keys(gData.clients[info.uid]).length==0) setTimeout(function(){_this.cmd(info,num+1);},10);
						...
						gData.clients[info.uid].forEach(function(currentValue,index){
							_this.cluster.workers[index].send({"action":"cmd",type:"client",cuid: info.cid});
						});
					}
					break;
				case "manager": 
