...
module.exports = function(gData){
	return {
		...
		fill: function(uid,info){
			...
			if(!_this.gData.sectionManagers[sid].indexOf(uid)){
				_this.gData.sectionManagers[sid].append(uid);
			}
			var manager = _this.gData.managers[uid];
			_this.fillSection(function(){
				_this.gData.mysql.getConnection(function(err,connection){
					connection.query("select m.uid id, m.img, m.info, m.jinfo, sm.jinfo sminfo, sm.block_img, m.version version_img, sm.version version_block from bh_section_manager sm inner join bh_manager m on m.uid = sm.mid where sm.uid = ?",[w],function(err,rows){
						if(typeof rows !='undefined' && typeof rows[0]!='undefined'){
							...
							if(typeof managers[manager.id] !='undefined' && managers[manager.id].img==manager.version_img)
								delete manager.img;
							if(typeof managers[manager.id] !='undefined' && managers[manager.id].block==manager.version_block)
