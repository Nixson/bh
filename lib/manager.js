	...
	return{
		...
		response: function(callback){
			...
			_this.Emitter.once('isResponse'+_this.session,function(){
				...
				process.send({ manager: {type: "exit", cluster: _this.globalData.cluster.worker.id, uid: _this.uid}});
				callback(_this.responseText,_this.header);
				if(_this.uid > 0){
					_this.globalData.timerOut[_this.uid] = setTimeout(function(){
						process.send({ manager: {type: "clear", cluster: _this.globalData.cluster.worker.id, uid: _this.uid}});
		...
		parse: function(){
			...
			_this.globalData.redis.get("bh:m:h:"+mUUID,function(err, reply){
				if(reply==null){
					...
					_this.globalData.manager[_this.uid] = _this;
					process.send({ manager: {type: "in", cluster: _this.globalData.cluster.worker.id, uid: _this.uid}});
				}
			});
		},
	}}
