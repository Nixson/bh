...
module.exports = function(gData){
	return {
		...
		fillAllSections: function(sid,uid,callback){
			var self = this;
			if(typeof self.gData.sectionManagers[sid]=='undefined')
				self.gData.sectionManagers[sid] = [];

			self.fillSection(sid,()=>{
				if(self.gData.sectionManagers[sid].indexOf(uid) == -1){
					self.gData.sectionManagers[sid].push(uid);
				}
				var cid = self.gData.sections[sid].cid;
				self.gData.mysql.getConnection(function(err,connection){
					connection.query("SELECT * from bh_section where cid = ? and uid != ?",[cid, sid],function(err,rows){
						if(typeof rows!='undefined' && typeof rows[0]!='undefined' && rows.length > 0){
							for(var nm in rows){
								...
								if(typeof self.gData.sectionManagers[section]=='undefined')
									self.gData.sectionManagers[section] = [];
								if(self.gData.sectionManagers[section].indexOf(uid) == -1){
									self.gData.sectionManagers[section].push(uid);
								}
							}
						}
						callback();
		...
		fill: function(uid,info){
			...
			_this.fillAllSections(sid,uid,function(){
