	...
	return {
		...
		fill: function(uid,info){
			...
			_this.fillSection(sid,function(){
				_this.gData.mysql.getConnection(function(err,connection){
					connection.query("select m.uid id, m.img, m.info, m.jinfo, sm.jinfo sminfo, sm.block_img, m.version version_img, sm.version version_block from bh_section_manager sm inner join bh_manager m on m.uid = sm.mid where sm.uid = ?",[uid],function(err,rows){
						if(typeof rows !='undefined' && typeof rows[0]!='undefined'){
