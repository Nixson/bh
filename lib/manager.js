	...
	return {
		...
		responseWs: function(info){
			...
			if(typeof _this.gData.managers[uid]=='undefined'){
				_this.gData.managers[uid] = {sid: "0",mList: []};
				_this.fill(uid,info);
			}
		...
		fill: function(uid,info){
			...
			_this.fillSection(sid,function(){
				_this.gData.mysql.getConnection(function(err,connection){
					connection.query("select m.uid id, m.img, m.info, m.jinfo, m.jimg, sm.jinfo sminfo, sm.block_img, m.version version_img, sm.version version_block from bh_section_manager sm inner join bh_manager m on m.uid = sm.mid where sm.mid = ?",[uid],function(err,rows){
						if(typeof rows !='undefined' && typeof rows[0]!='undefined'){
		...
		clean: function(uidHash){
			...
			if(typeof sid=='number')
				sid = sid.toString();
			console.log("clean managers",sid,typeof _this.gData.sectionManagers[sid]);
			if( typeof _this.gData.sectionManagers[sid]!='undefined'){
			...
			console.log("clean managers done",sid,typeof _this.gData.sectionManagers[sid]);
