	...
	return {
		...
		responseWs: function(info){
			...
			if(typeof _this.gData.managers[uid]=='undefined'){
				_this.gData.managers[uid] = {id: uid, cid: 0,sid: [],mList: []};
				_this.fill(uid,info);
			}else {
				...
				connection.query("select m.uid id, m.img, m.jinfo, m.jimg, sm.jinfo sminfo from bh_section_manager sm inner join bh_manager m on m.uid = sm.mid where sm.sid = ? and sm.mid = ?",[sid,mid],function(err,rows){
					if(typeof rows !='undefined' && typeof rows[0]!='undefined' && typeof rows[0].id !='undefined'){
						...
						if(typeof img.normal!='undefined')
							jMan.img = img.normal;
						callback(jMan.img,sminfo,false);
						delete jMan;
					}
					else {
		...
		get: function(cid,sid,mid,callback){
			var _this = this;
			...
			if(mid=="0"){
				if(typeof _this.gData.sectionManagers[cid]!='undefined'){
					mid = _this.gData.sectionManagers[cid][_this.random(0,_this.gData.sectionManagers[cid].length)];
				}
			}else {
				if(typeof _this.gData.sectionManagers[cid]!='undefined'){
					var find = false;
					for(num in _this.gData.sectionManagers[cid]){
						if(_this.gData.sectionManagers[cid][num]==mid){
							find = true;
							break;
						}
					}
			...
			_this.getInfo(sid,cid,mid,function(manager,error){
		...
		response: function(info, callback){
			...
			_this.gData.mList[info.session] = uidHash;
			if(typeof _this.gData.managers[uid]=='undefined'){
				_this.gData.managers[uid] = { id: uid, sid: [], cid:0, mList: []};
				_this.fill(uid,info);
			}else {
				var st = info.path.pathname.split('/');
				var query = info.path.query;
				var sidLine = st[1].split("i");
				var sid = parseInt(sidLine[1]);
				sid = sid.toString();
				var cid = parseInt(sidLine[0]);
				cid = cid.toString();
				if(typeof _this.gData.sectionManagers[cid]=='undefined')
					_this.gData.sectionManagers[cid] = [];
				if(_this.gData.sectionManagers[cid].indexOf(uid) == -1){
					_this.gData.sectionManagers[cid].push(uid);
					gData.signal.stat.managerIn(uid,sid,cid);
				}
			}
		...
		fillAllSections: function(cid,uid,callback){
			var self = this;
			if(typeof self.gData.sectionManagers[cid]=='undefined')
				self.gData.sectionManagers[cid] = [];

			if(self.gData.sectionManagers[cid].indexOf(uid) == -1){
				self.gData.sectionManagers[cid].push(uid);
			}
			if(typeof self.gData.sectionList[cid]=='undefined'){
				self.gData.sectionList[cid] = [];
			}
			if(self.gData.sectionList[cid].length==0)
				self.gData.mysql.getConnection(function(err,connection){
					connection.query("SELECT * from bh_section where cid = ?",[cid],function(err,rows){
						if(typeof rows!='undefined' && typeof rows[0]!='undefined' && rows.length > 0){
							for(var nm in rows){
								section = rows[nm]['uid'];
								if( typeof self.gData.sections[section] == 'undefined' || self.gData.sections[section].version < rows[nm].version){
									self.gData.sections[section] = rows[nm];
									var info = self.gData.sections[section].info;
									self.gData.sections[section].info = {};
									if(info!="")
										self.gData.sections[section].info = JSON.parse(info);
								}
								self.gData.sectionList[cid].push(section);
								self.loadInfo(section);
							}
						}
						connection.release();
						callback();
		...
		fillSection: function(sid, callback){
			...
			if(typeof sid=='number')
				sid = sid.toString();
			if( typeof _this.gData.bot[sid] != 'undefined')
				delete _this.gData.bot[sid];
		...
		fill: function(uid,info){
			...
			console.log("fill",_this.gData.managers[uid]);
			_this.fillAllSections(cid,uid,function(){
				_this.getInfo(sid,cid,uid);
			});

		},
		getManagerInfo: function(uid,cid,callback){
			console.log("manager.getManagerInfo",sid,cid,uid);
			var _this = this;
			if(typeof uid=='number')
				uid = uid.toString();
			if(uid=="0"){
			...
			_this.gData.mysql.getConnection(function(err,connection){
				connection.query("SELECT uid, fio, info, version FROM `bh_manager` where uid = ?",[uid],function(err,rows){
					if(typeof rows!='undefined' && typeof rows[0]!='undefined'){
						if(typeof _this.gData.managers[uid]=='undefined'){
							_this.gData.managers[rows[0].uid] = { sid: [], mList: []};
						}
						var manager = _this.gData.managers[rows[0].uid];
						manager.id = rows[0].uid;
						manager.uid = rows[0].uid;
						manager.cid = cid;
						manager.fio = rows[0].fio;
						manager.version_img = rows[0].version;
		...
		getInfo: function(sid,cid,uid,callback,reconnection){
			console.log("manager.getInfo",sid,cid,uid);
			var _this = this;
			...
			if(typeof reconnection == "undefined") {
				if(uid+""=="0"){
					...
					if(typeof _this.gData.managers[uid]!='undefined'){
						if(_this.gData.managers[uid].sid.indexOf(sid) >= 0 && typeof _this.gData.managers[uid].section !='undefined' && typeof _this.gData.managers[uid].section[sid] !='undefined'){
							if(typeof callback=='function'){
								callback(_this.gData.managers[uid],true);
							}
							return;
						}
					}
			...
			_this.gData.mysql.getConnection(function(err,connection){
				connection.query("select m.uid id, m.info, m.version version_img, sm.version version_block from bh_section_manager sm inner join bh_manager m on m.uid = sm.mid where "+W+"sm.sid = ?",[sid],function(err,rows){
						console.log("manager.getInfo.rows",rows.length);
						if(typeof rows !='undefined' && typeof rows[0]!='undefined' && typeof rows[0].id !='undefined'){
							if(typeof _this.gData.managers[rows[0].id]=='undefined')
								_this.gData.managers[rows[0].id] = { sid: [sid], cid: cid, mList: []};
							else{
							...
							console.log("manager.getInfo.version_img",rows[0].version_img);
		...
		infoSection: function(sid,cid,uid,callback){
			console.log("infoSection",sid,cid,uid);
			var _this = this;
			if(typeof _this.gData.managers[uid]=='undefined'){
				_this.getInfo(sid,cid,uid,callback);
				return;
			}else {
		...
		clean: function(uidHash,noOut){
			...
			var cid = _this.gData.managers[uid].cid;
			if(typeof sidArray=='number' || sidArray=="string")
				sidArray = [sidArray.toString()];

			if( typeof _this.gData.sectionManagers[cid]!='undefined'){
				if(_this.gData.sectionManagers[cid].indexOf(uid) >=0 )
					 _this.gData.sectionManagers[cid].splice(_this.gData.sectionManagers[cid].indexOf(uid),1);
				if(_this.gData.sectionManagers[cid].length == 0)
					delete _this.gData.sectionManagers[cid];
			}

			if( typeof _this.gData.sectionManagers[cid]=='undefined'){
				//delete sections;
				if(typeof _this.gData.sectionClientsOffline[cid] !='undefined'){
					for( var i in _this.gData.sectionClientsOffline[cid])
						_this.cleanClient(_this.gData.sectionClientsOffline[cid][i]);
					delete _this.gData.sectionClientsOffline[cid];
				}
				if(typeof _this.gData.sectionClientsMsgOffline[cid] !='undefined'){
					for( var i in _this.gData.sectionClientsMsgOffline[cid])
						_this.cleanClient(_this.gData.sectionClientsMsgOffline[cid][i]);
					delete _this.gData.sectionClientsMsgOffline[cid];
				}
			}
			...
			for(var sn in sidArray){
				...
				if(typeof noOut=='undefined' || !noOut)
					_this.gData.signal.stat.managerOut(uid,sid,cid);
				if( typeof _this.gData.sectionManagers[cid]=='undefined'){
					if( typeof _this.gData.sectionClients[cid]=='undefined'){
						if(typeof _this.gData.sections[sid]!='undefined'){
							if(typeof _this.gData.sections[sid].cid!='undefined'){
								delete _this.gData.triggers[_this.gData.sections[sid].cid];
								delete _this.gData.sectionList[_this.gData.sections[sid].cid];
							}
