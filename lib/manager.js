...
module.exports = function(gData){
	return {
		...
		bind: function(){
			...
			_this.cs = http.Server(function(request, response) {
							...
							if (request.method == 'POST') {
								...
								request.on('end', function () {
									...
									_this.response({session:uuid,path:info},function(resp,headerCode){
										...
										response.write(resp+"\n");
										response.end();
										var uid = _this.gData.mList[uuid];
										console.log(_this.gData.managers[uid].mList,_this.gData.managers[uid].mList.indexOf(uuid));
										_this.gData.managers[uid].mList.splice(_this.gData.managers[uid].mList.indexOf(uuid));
					...
					request.on('error', function() {
						_this.gData.Emitter.emit('isResponseManager'+uuid);
					});
		...
		responseWs: function(info){
			...
			if(typeof _this.queue[info.session] != 'undefined'){
				var pull = [];
				for(qNum in _this.queue[info.session]){
					pull.push(_this.queue[info.session][qNum]);
					_this.queue[info.session].splice(qNum,1);
				}
		...
		send: function(uid, msg){
			var _this = this;
			console.log("send:");
			if( typeof _this.gData.managers[uid].mList!='undefined' && _this.gData.managers[uid].mList.length > 0){
				var sessions = _this.gData.managers[uid].mList;
				for( var i in sessions){
					var sess = sessions[i];
					console.log("send",sess);
		...
		response: function(info, callback){
			...
			_this.gData.managers[uid].mList.push(info.session);
			if( info.path.type=='post' ){
				return true;
			}else {
				_this.isWs[info.session] = false;
			}
			if(typeof _this.queue[info.session] != 'undefined' && _this.queue[info.session].length > 0){
				var pull = [];
				for(qNum in _this.queue[info.session]){
					pull.push(_this.queue[info.session][qNum]);
					_this.queue[info.session].splice(qNum,1);
			...
			clearTimeout(_this.gData.mtimerOut[info.session]);
			_this.gData.Emitter.once('isResponseManager'+info.session,function(){
				clearTimeout(_this.globalTimeout[info.session]);
				clearTimeout(_this.gData.mtimerOut[info.session]);
				if(typeof _this.queue[info.session] != 'undefined' && _this.queue[info.session].length > 0){
					var pull = [];
					for(qNum in _this.queue[info.session]){
				...
				_this.gData.mtimerOut[info.session] = setTimeout(function(){
					_this.clean(uid,info.session);
					},_this.gData.config.timeout);
			});
			_this.globalTimeout[info.session] = setTimeout(function(){
				_this.gData.Emitter.emit('isResponseManager'+info.session);
		...
		clean: function(uid,sess){
			var _this = this;
			delete _this.queue[sess];
			if(typeof _this.gData.managers[uid].mList!='undefined' && _this.gData.managers[uid].mList.length > 0)
				return;
