...
module.exports = function(gData){
	return {
		...
		loadInfo: function(section,callback){
			...
			inText+='(function(){function bhelpLoad(url,name){if (window.XMLHttpRequest) xmlhttp=new XMLHttpRequest(); else xmlhttp=new ActiveXObject("Microsoft.XMLHTTP"); xmlhttp.onreadystatechange=function(){if (xmlhttp.readyState==4 && xmlhttp.status==200){if(window["localStorage"] === null) {localStorage.setItem(name,xmlhttp.responseText);} eval(xmlhttp.responseText);}}; xmlhttp.open("GET", url ); xmlhttp.send();}\nif(window["localStorage"] === null){var bhelpVersion = localStorage.getItem("bhelp_versioni"), bl="bhelp_latesti";var ba=bhelpSrvAddress+"/mini.js";\nconsole.log(bhelpSrvAddress); \nif(bhelpVersion==null || bhelpVersion != bhelpSrvVersion) bhelpLoad(ba,bl); else eval(localStorage.getItem(bl));} else bhelpLoad(ba,bl);})();';
			callback(inText,200);
		}
	}
}