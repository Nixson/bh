	...
	return {
		...
		getManager: function(uid,sid,callback){
			...
			if(mid == 0){
				...
				_this.gData.mysql.getConnection(function(err,connection){
					connection.query("select m.uid id, m.img, m.info from bh_section_manager sm inner join bh_manager m on m.uid = sm.mid where "+w,function(err,rows){
						console.log("select m.uid, m.img, m.info from bh_section_manager sm inner join bh_manager m on m.uid = sm.mid where "+w);
						console.log(rows);
						if(typeof rows !='undefined' && typeof rows[0]!='undefined'){
							...
							connection.query("select m.uid id, m.img, m.info from bh_section_manager sm inner join bh_manager m on m.uid = sm.mid where sm.sid = "+sid+" limit 1",function(err,rows){
								if(typeof rows !='undefined' && typeof rows[0]!='undefined'){
									var jMan = {};
									jMan.id = rows[0].id;
									jMan.img = rows[0].img;
		...
		loadInfo: function(section,callback){
			...
			if(typeof _this.gData.sections[section].info.bg =='undefined'){
				...
				_this.gData.mysql.getConnection(function(err,connection){
									connection.query("UPDATE bh_section SET info = :info where uid = :uid",{uid:section,info: JSON.stringify(_this.gData.sections[section].info)},function(err,result){
										connection.release();
									});
								});
			}
			if(typeof _this.gData.sections[section].info.off =='undefined'){
				var psNum = "";
				...
				_this.gData.sections[section].info.off = new Buffer(fs.readFileSync(_this.gData.config.dir.client+'/img/block/'+section+'-'+psNum+"."+_this.gData.sections[section].imgoff+".png")).toString('base64');
				_this.gData.mysql.getConnection(function(err,connection){
									connection.query("UPDATE bh_section SET info = :info where uid = :uid",{uid:section,info: JSON.stringify(_this.gData.sections[section].info)},function(err,result){
										connection.release();
									});
			...
			ret.lineImg = _this.gData.sections[section].info.off;
		...
		loadStatic: function(section,callback){
			var _this = this;
			var inText = 'var bhelpSrvAddress="https://static.bhelp.com",bhelpInfoAddress="https://gb.bhelp.com", bhelpSrvVersion='+_this.gData.sections[section].version+',bhelpSrvId='+section+',bhelpVer='+_this.gData.config.version+';\n';//+_this.gData.sections[section].version+";\n";
