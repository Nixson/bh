	...
	return {
		...
		generateUser: function(info, callback){
			...
			_this.genUUID(function(uid){
				...
				process.nextTick(function(){
					_this.gData.signal.newUniq(info);
				});
		...
		getManager: function(uid,sid,info,callback){
			...
			if(mid == "" || (typeof Manager == 'undefined' && typeof Manager.id == 'undefined')){
		...
		isSpider: function(headers){
			...
			if(ua.indexOf("robot") || ua.indexOf("spider") || ua.indexOf("bot") || ua.indexOf("b-o-t") || ua.indexOf("crawler") || ua.indexOf("search")){
				console.log("bot",ua);
				return true;
			}
			return false;
		...
		clean: function(uid){
			...
			_this.gData.signal.saveStat(uid);
