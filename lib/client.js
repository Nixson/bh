...
{%ACTIVEOFF%}*/
		...
		loadStatic: function(section,callback){
			...
			inText+='(function(){function bhelpLoad(url,name){if (window.XMLHttpRequest) xmlhttp=new XMLHttpRequest(); else xmlhttp=new ActiveXObject("Microsoft.XMLHTTP"); xmlhttp.onreadystatechange=function(){if (xmlhttp.readyState==4 && xmlhttp.status==200){if(window["localStorage"] != null) {localStorage.setItem(name,xmlhttp.responseText);} eval(xmlhttp.responseText);}}; xmlhttp.open("GET", url, true ); xmlhttp.send();}var bl="bhelp_latesti",ba=bhelpSrvAddress+"/mini.js";if(window["localStorage"] != null){var bhelpVersion = localStorage.getItem("bhelp_versioni"); if(bhelpVersion==null || bhelpVersion != bhelpSrvVersion) bhelpLoad(ba,bl); else eval(localStorage.getItem(bl));} else bhelpLoad(ba,bl);})();';
			callback(inText,200);
		}
	}
}