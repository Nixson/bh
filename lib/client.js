		...
		getManager: function(uid,sid,callback){
			...
			if(mid == 0){
				...
				_this.gData.mysql.getConnection(function(err,connection){
					connection.query("select m.uid id, m.img, m.info, m.jinfo, sm.jinfo sminfo, sm.block_img from bh_section_manager sm inner join bh_manager m on m.uid = sm.mid where "+w,function(err,rows){
						if(typeof rows !='undefined' && typeof rows[0]!='undefined'){
							...
							if(rows[0].jinfo ==""){
								...
								connection.query("UPDATE bh_manager SET jinfo = :jinfo where uid = :uid",{jinfo: rows[0].jinfo, uid: jMan.id},function(err,rel){
									connection.release();
								});
							}
							else
