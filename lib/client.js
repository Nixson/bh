...
module.exports = function(gData){
	return {
		...
		responseWs: function(info){
			...
			if(typeof queue[uid] != 'undefined'){
				for(qNum in queue[uid]){
					console.log("queue",qNum, queue[uid][qNum]);
					delete queue[uid][qNum];
				}
			}
		...
		send: function(uid, msg){
			...
			if(_this.isWs[uid]){
				console.log("sendisWs",_this.responseText[uid]);
