	...
	return {
		...
		getManager: function(uid,sid,info,callback){
			...
			if(mid == "" || typeof Manager.id == 'undefined'){
				_this.gData.manager.get(sid,_this.gData.clients[uid].mid,function(rManager,err){
					if(err){
						...
						jMan.version_img = rManager.version_img;
						jMan.version_block = rManager.section[sid];
						var loadImg = false;
				...
				if(_this.gData.clients[uid].mid!="0")
					w = 'sm.sid = '+sid+' and sm.mid = '+_this.gData.clients[uid].mid;
				_this.gData.mysql.getConnection(function(err,connection){
					connection.query("select m.uid id, m.img, m.info, m.jinfo, m.jimg, sm.jinfo sminfo, sm.block_img, m.version version_img, sm.version version_block from bh_section_manager sm inner join bh_manager m on m.uid = sm.mid where "+w,function(err,rows){
						if(typeof rows !='undefined' && typeof rows[0]!='undefined' && typeof rows[0].id !='undefined'){
							...
							if(typeof _this.gData.managers[jMan.id] !='undefined' && _this.gData.managers[jMan.id].img==jMan.version_img)
								delete jMan.img;
							if(position==psNum && typeof _this.gData.managers[jMan.id] !='undefined' && _this.gData.managers[jMan.id].block==jMan.version_block)
								delete jMan.block_img;
							Call.manager = jMan;
							Call.position = psNum;
							callback(JSON.stringify(Call),200);
				...
				_this.gData.manager.infoSection(sid,Manager.id,function(jManager){
					var psNum = "";
					switch(_this.gData.sections[sid].ps){
						case 1:case 2: psNum = "12"; break;
						case 3: psNum = "3"; break;
					...
					if(loadImg)
						_this.gData.manager.img(sid,jMan.id,function(img,sminfo,err){
							console.log("manager.img" ,typeof sminfo, typeof sminfo[psNum], psNum);
							jMan.img = img;
							jMan.block_img = sminfo[psNum];
							callback(JSON.stringify(Call),200);
						});
		...
		fill: function(uid,info,firts){
			...
			if(client.region=='' || client.city=='' || info.path.headers.geoip_latitude.substr(-4)=='0000'){
				...
				if(typeof location.region.name!='undefined' && typeof location.region.name.en!='undefined')
					client.region = location.region.name.en;
				else
					isGeo = true;
				if(typeof location.city.name!='undefined' && typeof location.city.name.en!='undefined')
				...
				if(typeof location.latitude!='undefined' && typeof location.longitude!='undefined')
					...
					isGeo = true;
				if(typeof location.country.name!='undefined' && typeof location.country.name.en!='undefined')
					client.country_name = location.country.name.en;
				else
					isGeo = true;
				if(typeof location.country.iso!='undefined')
					client.country_code = location.country.iso;
			}
			if(isGeo){
				console.log(info.path.headers);
		...
		clean: function(uid){
			...
			if( typeof _this.gData.sectionManagers[sid]=="undefined" || _this.gData.sectionManagers[sid].length == 0){
				if( typeof _this.gData.msg[uid] !='undefined'){
				...
				_this.gData.sectionClientsOffline[sid].push(uid);
