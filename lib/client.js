	...
	return {
		...
		getManager: function(uid,sid,callback){
			...
			if(mid == 0){
				...
				_this.gData.mysql.getConnection(function(err,connection){
					connection.query("select m.uid id, m.img, m.info, m.jinfo from bh_section_manager sm inner join bh_manager m on m.uid = sm.mid where "+w,function(err,rows){
		...
		loadInfo: function(section,callback){
			...
			if(typeof _this.gData.sections[section].info["off"+psNum] =='undefined'){
				...
				_this.gData.mysql.getConnection(function(err,connection){
					console.log(JSON.stringify(_this.gData.sections[section].info));
									connection.query("UPDATE bh_section SET info = ':info' where uid = :uid",{uid:section,info: JSON.stringify(_this.gData.sections[section].info)},function(err,result){
