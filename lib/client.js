		...
		getManager: function(uid,sid,callback){
			...
			if(mid == 0){
				...
				_this.gData.mysql.getConnection(function(err,connection){
					connection.query("select m.id, m.img, m.info from bh_section_manager sm inner join bh_manager m on m.id = sm.mid where "+w,function(err,rows){
						console.log("select m.id, m.img, m.info from bh_section_manager sm inner join bh_manager m on m.id = sm.mid where "+w);
						console.log(rows);
						if(typeof rows !='undefined' && typeof rows[0]!='undefined'){
							...
							connection.query("select m.id, m.img, m.info from bh_section_manager sm inner join bh_manager m on m.id = sm.mid where sm.sid = "+sid+" limit 1",function(err,rows){
								if(typeof rows !='undefined' && typeof rows[0]!='undefined'){
									var jMan = {};
									jMan.id = rows[0].id;
									jMan.img = rows[0].img;
