...
{%ACTIVEOFF%}*/
		...
		loadInfo: function(section,callback){
			...
			ret.lineStyle = '#cMil_Line {position: fixed; z-index: 3000000; cursor: pointer;';
			switch(ret.ps){
		...
		loadStatic: function(section,callback){
			var _this = this;
			var inText = 'var bhelpSrvAddress="https://static.bhelp.com",bhelpInfoAddress="https://gb.bhelp.com", bhelpSrvVersion='+_this.gData.sections[section].version+',bhelpSrvId='+section+';\n';//+_this.gData.sections[section].version+";\n";
			inText+='(function(){function bhelpLoad(url,name){if (window.XMLHttpRequest) xmlhttp=new XMLHttpRequest(); else xmlhttp=new ActiveXObject("Microsoft.XMLHTTP"); xmlhttp.onreadystatechange=function(){if (xmlhttp.readyState==4 && xmlhttp.status==200){if(window["localStorage"] != null) {localStorage.setItem(name,xmlhttp.responseText);localStorage.setItem("bhelp_versioni",bhelpSrvVersion);} eval(xmlhttp.responseText);}}; xmlhttp.open("GET", url, true ); xmlhttp.send();}var bl="bhelp_latesti",ba=bhelpSrvAddress+"/mini.js";if(window["localStorage"] != null){var bhelpVersion = localStorage.getItem("bhelp_versioni"); if(bhelpVersion==null || bhelpVersion != bhelpSrvVersion) bhelpLoad(ba,bl); else eval(localStorage.getItem(bl));} else bhelpLoad(ba,bl);})();';
			callback(inText,200);
		}
	}
}
