				...
				for(var manager in self.gData.managers[section]){
					...
					for( var num in self.gData.managers[section][manager]["in"]){
						if(self.gData.managers[section][manager]["in"][num] >= timeLast && self.gData.managers[section][manager]["in"][num] <= nowH){
							appMng.push({data:self.gData.managers[section][manager]["in"][num],type:"in"})
							appIn++;
							lastInNow = self.gData.managers[section][manager]["in"][num];
						}
						else if(self.gData.managers[section][manager]["in"][num] < timeLast)
							lastIn = self.gData.managers[section][manager]["in"][num];
						else if(self.gData.managers[section][manager]["in"][num] > nowH){
							appMngNow.push({data:self.gData.managers[section][manager]["in"][num],type:"in"})
							appInNow++;
						}
					}
					...
					for( var num in self.gData.managers[section][manager]["out"]){
						if(self.gData.managers[section][manager]["out"][num] >= timeLast && self.gData.managers[section][manager]["in"][num] <= nowH){
							...
							appMng.push({data:self.gData.managers[section][manager]["out"][num],type:"out"})
							appOut++;
							lastOutNow = self.gData.managers[section][manager]["out"][num];
						}
						else if(self.gData.managers[section][manager]["out"][num] < timeLast)
							lastOut = self.gData.managers[section][manager]["out"][num];
						else if(self.gData.managers[section][manager]["out"][num] > nowH){
							appMngNow.push({data:self.gData.managers[section][manager]["out"][num],type:"out"})
							appOutNow++;
						}
					}
					...
					if(appIn == 0 && appOut == 0){
						console.log("appIn",appIn,appOut,lastOut,lastIn);
						if(lastOut == 0 && lastIn == 0)
							self.gData.managers[section][manager]["hour"][timeLast] = 0;
						else {
							if(lastOut > lastIn)
								self.gData.managers[section][manager]["hour"][timeLast] = 0;
							else
								self.gData.managers[section][manager]["hour"][timeLast] = 3600;
						}
					}else {
						var summ = 0;
						var start = timeLast;
...
//						console.log("appIn",appIn,appOut,lastOut,lastIn,appMng);
						for( var ob in appMng){
							console.log(lt,appMng[ob].type,summ);
						...
						self.gData.managers[section][manager]["hour"][timeLast] = summ;
					}

					if(appInNow == 0 && appOutNow == 0){
						if(lastOutNow == 0 && lastInNow == 0)
							self.gData.managers[section][manager]["hour"][nowH] = 0;
						else {
							if(lastOut > lastIn)
								self.gData.managers[section][manager]["hour"][nowH] = 0;
							else
								self.gData.managers[section][manager]["hour"][nowH] = self.gData.time() - nowH;
						}
					}else {
						var summ = 0;
						var start = nowH;
						...
						self.gData.managers[section][manager]["hour"][nowH] = summ;
					}

				}
				//_this.add({'section':section,'muid':manager,'attr':"offdTimeC.maxd",'hTime':hTime,'value':0});
			}
			console.log(self.gData.managers["1873"]["2488"]["in"]);
			console.log(self.gData.managers["1873"]["2488"]["out"]);
			console.log(self.gData.managers["1873"]["2488"]["hour"]);
		},
		...
		cmanager: function(section,manager){
			...
			if(typeof this.gData.managers[section][manager]=='undefined')
				this.gData.managers[section][manager] = {"in":[],"out":[],"hour":{}};
		},
		manager: function(sc,section,manager){
			this.cmanager(section,manager);
			switch(sc){
				case "in":
							this.gData.managers[section][manager]["in"].push(this.gData.time());
				case "out":
							this.gData.managers[section][manager]["out"].push(this.gData.time());
			}
		},
