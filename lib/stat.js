...
module.exports = function(gData){
	return {
							...
							switch(path[2]){
								...
								case "online":
											if(typeof path[3]!="undefined")
												_this.online(path[3],info.query.info);
											break;
								case "offline":
											if(typeof path[3]!="undefined")
												_this.offline(path[3],info.query.info);
											break;
							}
				...
				for(var manager in self.gData.log.managers[section]){
					...
					for( var num in self.gData.log.managers[section][manager]["in"]){
						if(self.gData.log.managers[section][manager]["in"][num] >= timeLast && self.gData.log.managers[section][manager]["in"][num] <= nowH){
							...
							self.gData.log.managers[section][manager]["in"].splice(num,1);
							if(s > lastIn)
								lastIn = s;
						}
					...
					for( var num in self.gData.log.managers[section][manager]["out"]){
						if(self.gData.log.managers[section][manager]["out"][num] >= timeLast && self.gData.log.managers[section][manager]["in"][num] <= nowH){
							...
							self.gData.log.managers[section][manager]["out"].splice(num,1);
							if(s > lastOut)
								lastOut = s;
						}
		...
		get: function(section,callback){
			...
			if( typeof self.gData.log.managers=='undefined'){
				self.gData.log.managers = {};
			}
			if(typeof _this.gData.log[section]=="undefined" && self.gData.log.managers[section]=='undefined'){
				callback("{}");
			}
			else{
				...
				for(var i = 0; i < 7; i++){
					veekList.push({"range":{"in": hTime - ((7 - i)*day),"out": hTime - ((6 - i)*day)},log:{},section:{}});
				}
				for(var manager in _this.gData.log[section]){
					for(var i = 0; i < 7; i++){
						veekList[i].log[manager] = {};
					}
					for(var typeName in _this.gData.log[section][manager]){
						for(var i = 0; i < 7; i++){
							veekList[i].log[manager][typeName] = 0;
						}
						var list = _this.gData.log[section][manager][typeName];
						for(var dTime in list){
							var dNum = 6 - Math.floor((hTime - parseInt(dTime))/day);
							if(dNum < 0)
								continue;
							veekList[dNum].log[manager][typeName]+=list[dTime];
						}
					}
				}
				for(var dTime in _this.gData.log.sections[section]){
					var dNum = 6 - Math.floor((hTime - parseInt(dTime))/day);
					if(dNum < 0)
						continue;
					for( var tName in _this.gData.log.sections[section][dTime]){
						if(typeof veekList[dNum].section[tName]=='undefined')
							veekList[dNum].section[tName] = _this.gData.log.sections[section][dTime][tName];
						else
							veekList[dNum].section[tName] += _this.gData.log.sections[section][dTime][tName];
					}

				}
				/*console.log(hTime,veekList);
				rList.log = _this.gData.log[section];
				rList.sections = _this.gData.log.sections[section];*/
				//console.log(_this.gData.log[section]);
				callback(JSON.stringify(veekList));
			}
		},
		...
		manager: function(sc,section,manager){
			...
			switch(sc){
				case "in":
							self.gData.log.managers[section][manager]["in"].push(this.gData.time());
							break;
				case "out":
							self.gData.log.managers[section][manager]["out"].push(this.gData.time()-10);
							break;
			}
		},
		user: function(sc,section,query){
			...
			switch(sc){
				case "add":
							...
							if(typeof info.novisitor=="undefined")
								self.gData.log.sections[section][hTime].visitor++;
							if(info.online==1)
								self.gData.log.sections[section][hTime].online++;
							else
								self.gData.log.sections[section][hTime].offline++;
							console.log("add",info,self.gData.log.sections[section]);
							break;
				case "uniq":
...
//			console.log(self.gData.log.sections);
		...
		loadManagerList: function(section,callback){
			...
			_this.gData.mysql.getConnection(function(err,connection){
				connection.query("select sm.mid id from bh_section_manager sm where sm.sid = ?",[section],function(err,rows){
					connection.release();
					if(typeof rows !='undefined' && typeof rows[0]!='undefined' && typeof rows[0].id !='undefined'){
						for(var num in rows){
							var manager = rows[num].id.toString();
							if(typeof _this.gData.log[section][manager]=='undefined')
								_this.gData.log[section][manager] = {};
							if(typeof _this.gData.log.managers[section][manager]=='undefined')
								_this.gData.log.managers[section][manager] = {"in":[],"out":[]};
						}
					}
					callback();
...
//			if(typeof section==')
			...
			console.log(section, typeof section,typeof info.manager, info);
			...
			switch(info.action){
				case "add":
							for(var managerUid in _this.gData.log[section]){
								console.log(managerUid, typeof managerUid);
		...
		*/
		...
		online:  function(section,info){/*manager,action,*/
			...
			_this.add({'section':section,'muid':info.manager,'attr':"ondTime.cntd",'hTime':hTime,'value':0});
			...
			if(info.reaction){
				...
				else if(_this.gData.log[section][info.manager]['ondTime.mind'][hTime] > info.sReaction)
					_this.gData.log[section][info.manager]['ondTime.mind'][hTime] = info.sReaction;
				if(_this.gData.log[section][info.manager]['ondTime.maxd'][hTime] < info.sReaction)
				...
				_this.add({'section':section,'muid':managerUid,'attr':"onCntAdviceNoadviceThisManager",'hTime':hTime,'value':1});
			/*
...
*/
		...
		clean: function(){
			...
			var day = 24*3600;
			var lastVeek = day*8;
			for(var ndt in self.gData.log){
				if(ndt=="managers" ||  ndt=="sections"){
					continue;
				}
				var section = self.gData.log[ndt];
				for(var mNum in section){
					var managers = section[mNum];
					for (var tNum in managers) {
						for (var dTime in managers[tNum]) {
							var dNum = parseInt(dTime);
							if(dNum < lastVeek)
								delete managers[tNum][dTime];
						}
					}
				}
			}
			if(typeof self.gData.log.sections!='undefined'){
				for(var ndt in self.gData.log.sections){
					var section = self.gData.log[ndt];
					for(var dTime in section){
						var dNum = parseInt(dTime);
						if(dNum < lastVeek)
								delete section[dTime];
					}
				}
			}
		},
