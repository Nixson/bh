				...
				for(var manager in self.gData.managers[section]){
					...
					for( var num in self.gData.managers[section][manager]["in"]){
						if(self.gData.managers[section][manager]["in"][num] >= timeLast && self.gData.managers[section][manager]["in"][num] <= nowH){
							...
							var s = self.gData.managers[section][manager]["in"][num];
							delete self.gData.managers[section][manager]["in"][num];
							if(s > lastIn)
								lastIn = s;
						}
					...
					if(lastIn > 0)
						self.gData.managers[section][manager]["in"].push(lastIn);
					...
					for( var num in self.gData.managers[section][manager]["out"]){
						if(self.gData.managers[section][manager]["out"][num] >= timeLast && self.gData.managers[section][manager]["in"][num] <= nowH){
							...
							var s = self.gData.managers[section][manager]["out"][num];
							delete self.gData.managers[section][manager]["out"][num];
							if(s > lastOut)
								lastOut = s;
						}
					...
					if(lastOut > 0)
						self.gData.managers[section][manager]["out"].push(lastOut);
...
*/
		...
		add: function(info){
			...
			if(typeof info.rewrite!='undefined'){
				_this.gData.log[info.section][info.muid][info.attr][info.hTime] = value;
			}
			else{
				if(info.value > 0)
