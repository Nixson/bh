...
module.exports = function(gData){
	var info = {
		...
		stat: {
			parent: null,
			uniq: function(sid){
				console.log('stat uniq',sid,this.parent.statOption);

//				http.request()
			},
			save: function(uid){
				console.log('stat save',uid,this.parent.statOption);
				//засеч время на сайте, просмотренные страницы
				//проверить открытые и закрытые чаты (online reaction)
			},
			userIn: function(uid){
				console.log('stat userIn',uid,this.parent.statOption);
			},
			online: {
				parent: null,
				add: function(uid){
					console.log('stat online add chat',uid,this.parent.statOption);
					//проверить активатор
				}
			},
			offline: {
