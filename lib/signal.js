...
module.exports = function(gData){
	return {
		...
		saveOffline: function(uid, query){
			...
			client.cname = query.offlinePhone;
			_this.gData.mysql.getConnection(function(err,connection){
				connection.query( 'select uid from bh_customer where uid = ?',[uid], function(err, rows) {
					var post = {sid: info.sid, info: JSON.stringify(client), msg: msg, hinfo: hinfo};


					if(typeof rows!='undefined' && typeof rows[0]!='undefined'){

						connection.query( "UPDATE bh_customer SET ? WHERE uid = "+uid, post);
						console.log('update');
					}
					else {
						console.log('insert');
