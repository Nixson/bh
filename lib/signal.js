...
module.exports = function(gData){
	return {
		...
		getMsg: function(mid,sid,cid){
			var _this = this;
			if(typeof _this.gData.msg[cid] == "undefined" && !_this.gData.clients[cid].find){
		...
		loadClientsOffline: function(sid){
			var _this = this;
						...
						if(typeof rows!='undefined' && typeof rows[0]!='undefined'){
							for(var num in rows){
								...
								if( typeof _this.gData.clients[uid]=='undefined' ){
									...
									_this.gData.clients[uid] = client;
							...
							_this.getClientsOffline(sid);
						}
					});
			});
		},
		loadClientsMsgOffline: function(sid){
			var _this = this;
			...
			_this.gData.mysql.getConnection(function(err,connection){
					connection.query( 'select uid, sid, info, offmsg from bh_customer where sid = ? and offmsg != "" order by uid desc limit ?',[sid,limit], function(err, rows) {
						...
						if(typeof rows!='undefined' && typeof rows[0]!='undefined'){
							...
							_this.getClientsMsgOffline(sid);
						}
					});
			});
		},
		getClientsOffline: function(sid){
			var _this = this;

		},
		getClientsMsgOffline: function(sid){
			var _this = this;
		},
			...
			_this.gData.mysql.getConnection(function(err,connection){
				connection.query( 'select uid, offmsg from bh_customer where uid = ?',[uid], function(err, rows) {
					...
					if(typeof rows!='undefined' && typeof rows[0]!='undefined'){
						...
						offline.push({uid: "f"+(offline.length+1),data:_this.gData.time(),text:query.offlineText,status:0,who:3});
						...
						post.uid = uid;
						var offline = [];
						offline.push({uid: "f1",data:_this.gData.time(),text:query.offlineText,status:0,who:3});
