...
module.exports = function(gData){
	return {
		...
		triggerAction: function(uid,info){
			...
			for( id in info){
				var trigger = info[id];
				console.log(trigger,client.triggers);
				if(client.triggers.indexOf(trigger.uid) && trigger.safe==1)
					continue;
				triggerCondition(uid,client,trigger);
			}
		...
		triggerCondition: function(uid,client,trigger){
			...
			if(trigger.condition.length() == 0)
				return;
			var type = 0;
			if(trigger.condition[0]=='and')
				type = 1;
			var action = true;
			if(trigger.condition.length() > 1){
...
<select name="trigger[condition][key][]" class="span5" data-type="key">
        <option value="hour_of_day">Время дня</option>
        <option value="day_of_week">День недели</option>
        <option value="visitor_time_on_page">Секунд на странице</option>
        <option value="visitor_time_on_site">Секунд на сайте</option>
        <option value="visitor_ip">IP адрес</option>
        <option value="visitor_region">Регион посетителя</option>
        <option value="visitor_city">Город посетителя</option>
        <option value="country_code">Код страны</option>
        <option value="cnt_session">Посещений сайта</option>
        <option value="cnt_chat">Разговоров с оператором</option>
        <option value="url">Адрес страницы</option>
        <option value="title">Заголовок страницы</option>
        <option value="cnt_page">Количество просмотренных страниц</option>
        <option value="last_page">Предыдущая страница</option>
        <option value="name">Имя клиента</option>
        <option value="email">E-mail клиента</option>
        <option value="phone">Телефон клиента</option>
        <option value="ref">Поисковая система</option>
