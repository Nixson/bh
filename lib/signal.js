		...
		triggerCondition: function(uid,client,trigger){
			...
			if(action){
				if(trigger.action.actions.length > 0){
					_this.triggerAction(uid, client, trigger.action.actions);
				}
			}


		},
		triggerAction: function(uid,client,actions){
/*
<select name="trigger[actions][key][]" class="span5" data-type="key">
                  <option value="wait">Ждать, сек</option>
                  <option value="sendMessageToVisitor">Отправить сообщение клиенту</option>
                  <option value="sendMessageToConsultant">Отправить сообщение консультанту</option>
                  <option value="addTag">Добавить тег</option>
                  <option value="rmTag">Убрать тег</option>
                  <option value="editName">Задать имя</option>
                  <option value="editComment">Добавить примечание</option>
                </select>

*/
			console.log(uid,actions);
		},
