	...
	return {
		...
		signalManager: function(uid,info){
			...
			process.nextTick(function(){
				switch(info.path.query.manager){
					...
					case "SendMsg": 
					console.log(info.path.query);
					_this.sendMsg(info.path.query.to,parseInt(st[1]),info.path.query.info); break;
				}
			});
		},
		...
		message: function(cid,query){
			...
			if( typeof _this.gData.msg[cid][query.uid] == 'undefined') {
				...
				console.log("message",_this.gData.msg[cid][query.uid],query.uid);
			...
			for( Manager in ManagerList ){
				var muid = ManagerList[Manager];
				if(typeof _this.gData.managers[muid] !='undefined'){
						_this.gData.manager.send(muid,{action: "msg", info: {uid: cid, msg: _this.gData.msg[cid][query.uid]} });
				}
			}
		},
		sendMsg: function(cid,sid,msg){
			console.log(msg);
			var _this = this;
			_this.gData.client.sendMsg(cid,msg);
			if(typeof _this.gData.sectionManagers[sid]=='undefined') return;
			var ManagerList = _this.gData.sectionManagers[sid];
			for( Manager in ManagerList ){
				var muid = ManagerList[Manager];
				if(typeof _this.gData.managers[muid] !='undefined'){
						_this.gData.manager.send(muid,{action: "msg", info: {uid: cid, msg: _this.gData.msg[cid][msg.uid]} });
				}
			}
		},
