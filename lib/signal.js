	...
	return {
		...
		bot: function(uid,info){
			...
			if(typeof this.gData.bot[sid] =='undefined')
				this.loadBot(sid,function(){_this.activeBot(sid,uid);});
			else
				_this.activeBot(sid,uid);
		},
		...
		activeBot: function(sid,uid){
			var _this = this;
			process.nextTick(function(){
				// 1. Проверка онлайн
				if(typeof _this.gData.sectionManagers[sid]!='undefined'){
					if(typeof _this.gData.msg[uid] == 'undefined' && typeof _this.gData.bot[sid][1]!='undefined') {
						uidMsg = "m1";
						var msg = {
							uid: uidMsg,
					...
					if(typeof _this.gData.msg[uid] != 'undefined' &&  _this.gData.bot[sid][2]!='undefined'){
						var cnt = 0;
						var cntU = 0;
		...
		saveOffline: function(uid, query){
			...
			console.log("saveOffline cname",client.cname);
