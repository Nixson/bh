...
module.exports = function(gData){
	var info = {
		...
		signalManager: function(uid,info){
			var st = qs.unescape(info.path.pathname).split('/');
			var _this = this;
			var sid = parseInt(st[1]).toString();
			if(sid=="0" || sid==""){
				_this.responseText[uid] = JSON.stringify({error:"Not correct sid"});
				_this.header[uid] = 404;
				_this.gData.Emitter.emit('isResponseSignal'+uid);
			}
			process.nextTick(function(){
				switch(info.path.query.manager){
					case "List": _this.getUsers(uid,sid); break;
					case "Msg": _this.getMsg(uid,sid,info.path.query.info); break;
					case "MsgOffline": _this.getMsgOffline(uid,sid,info.path.query.info); break;
					case "SendMsg": 
							_this.sendMsg(info.path.query.to,sid,info.path.query.info);
							_this.gData.Emitter.emit('isResponseSignal'+uid);
					break;
		...
		signalClient: function(uid,info){
			...
			if(typeof query.offlineText != 'undefined'){
				...
				if(typeof _this.clients[uid]!='undefined')
					_this.clients[uid].active = 1;
				_this.gData.Emitter.emit('isResponseSignal'+uid);
				return;
			}
		...
		message: function(cid,query){
			...
			if(typeof sid == 'number')
				sid = sid.toString();
			if(sid=='0') return;
			_this.sendManagers(sid,{action: "msg", info: {uid: cid, msg: _this.gData.msg[cid][last].list[query.uid]} });
		},
		sendMsg: function(cid,sid,msgNj){
			...
			_this.sendManagers(sid,{action: "msg", info: {uid: cid, msg: _this.gData.msg[cid][last].list[msg.uid]} });
		},
		...
		stat: {
			...
			userIn: function(uid,novisitor){
				...
				var client = _this.gData.clients[uid];
				var online = 0;
				if(typeof _this.gData.sectionManagers[client.sid]!='undefined')
					online = 1;
				var statOption = {
						host: "127.0.0.1",
						port: gData.config.srv.stat,
			...
			managerIn: function(uid, sid){
				...
				req.write('info={}');
				req.end();
				console.log('stat managerIn',uid);
				var cnt = 0;
				if(typeof _this.gData.sectionManagers[sid]!='undefined')
					cnt = _this.gData.sectionManagers[sid].length;
				if(typeof _this.gData.sectionClients[sid]!='undefined' && cnt == 1){
					var clients = _this.gData.sectionClients[sid];
					for(var cn in clients){
						_this.stat.userIn(clients[cn],true);
					}
				}
			},
			managerOut: function(uid,sid){
				var _this = this.parent;
				var statOption = {
						host: "127.0.0.1",
						port: gData.config.srv.stat,
				...
				req.write('info={}');
				req.end();
				console.log('stat managerOut',uid);
				var cnt = 0;
				if(typeof _this.gData.sectionManagers[sid]!='undefined')
					cnt = _this.gData.sectionManagers[sid].length;
				if(typeof _this.gData.sectionClients[sid]!='undefined' && cnt==0){
					var clients = _this.gData.sectionClients[sid];
					for(var cn in clients){
						_this.stat.userIn(clients[cn],true);
					}
